...
private:
    ...
    //hash table is an array of pointers that point to nodes, which would point to other nodes
    Node** hashTable;
    //this is the capacity
    unsigned int numBuckets;
    //this is n for load factor
    unsigned int numKeyValuePairs;
    

    void initHashTable(Node**& hashTable, unsigned int numBuckets);
    unsigned int hashMapIndex(const std::string& key) const;
    void reMapHashTable();
};
